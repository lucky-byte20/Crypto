function startCTRSimulation() {
    ...
    plaintextBlocks.forEach((block, index) => {
        ...
        blockContainer.style.opacity = "0"; // Start hidden
        blockContainer.style.transition = "opacity 0.5s"; // Smooth fade-in
        blockContainer.style.marginTop = "100px"; // Space between blocks
       ...
       block.split('').forEach(char => {
        ...
        spacer.style.height = "20px"; // Or use margin in CSS
...
function generateIncrementedCounter(counterValue) {
    const numericString = counterValue.toString().padStart(16, '0'); // 16-digit number
    const byteArray = numericString.split('').map(d => d.charCodeAt(0)); // convert digits to char bytes
